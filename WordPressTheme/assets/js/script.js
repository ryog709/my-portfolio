...
jQuery(function ($) {
  ...
  var wipeAnimations = document.querySelectorAll('.js-wipeAnimation');
  // 各要素に対してアニメーションを適用
  wipeAnimations.forEach(function (element) {
    gsap.fromTo(element, {
      clipPath: 'inset(0 100% 0 0)'
    }, {
      clipPath: 'inset(0 0% 0 0)',
  ...
  /* フェードインアニメーション */
  var fadeUps = document.querySelectorAll('.js-fadeUp');
  // 各要素に対してアニメーションを適用
  fadeUps.forEach(function (element) {
    gsap.fromTo(element, {
      autoAlpha: 0,
      y: 200
    }, {
  ...
  // マウスストーカー
  // 'stalker'クラスを持つ要素を選択
  var circle = document.querySelector('.js-stalker');
  // サークルのx座標を変更するためのクイックアニメーションを作成
  var xTo = gsap.quickTo(circle, 'x', {
    duration: .5,
    ease: 'power3'
  });
  // サークルのy座標を変更するためのクイックアニメーションを作成
  var yTo = gsap.quickTo(circle, 'y', {
    duration: .5,
    ease: 'power3'
  });
